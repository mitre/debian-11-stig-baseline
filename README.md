# Debian 11 (bullseye) STIG-Ready Baseline InSpec Profile
...
This profile is therefore an InSpec overlay of the [Canonical Ubuntu 20.04 LTS STIG baseline](https://github.com/mitre/canonical-ubuntu-20.04-lts-stig-baseline) maintained by the MITRE SAF team. It runs the full upstream control set against Debian 11 and overrides, in `controls/overlay.rb`, only the checks where Debian genuinely differs from Ubuntu — different packaging conventions, different file locations, or requirements with no Debian equivalent. Ubuntu 20.04 was chosen for Debian 11 because its component set (OpenSSL 1.1.1, matching systemd/PAM era) is the closest STIG-covered match to bullseye.
...
All inputs declared by the upstream Canonical Ubuntu 20.04 LTS STIG baseline remain available and keep their upstream defaults — set any of them at run time exactly as you would when running the Ubuntu profile. See the [upstream `inspec.yml`](https://github.com/mitre/canonical-ubuntu-20.04-lts-stig-baseline/blob/main/inspec.yml) for the full list.
...
```bash
# Using `ssh` transport
bundle exec [inspec or cinc-auditor] exec https://github.com/mitre/debian-11-stig-baseline/archive/main.tar.gz --input-file=<your_inputs_file.yml> -t ssh://<hostname>:<port> --sudo --reporter=cli json:<your_results_file.json>
```
...
mkdir profiles
cd profiles
git clone https://github.com/mitre/debian-11-stig-baseline.git
bundle exec [inspec or cinc-auditor] archive debian-11-stig-baseline
...
```bash
cd debian-11-stig-baseline
git pull
cd ..
bundle exec [inspec or cinc-auditor] archive debian-11-stig-baseline --overwrite
```
...
Several controls in the upstream Ubuntu STIG concern FIPS 140 cryptography (SV-238216, SV-238217, SV-238325, SV-255912, SV-238363). It is possible to run the original Ubuntu profile checks against Debian and receive results, but note that even if the correct packages are present, Debian has no *validated* FIPS modules.
...
Therefore, even though Debian can technically be configured to pass the *literal text* of Ubuntu's STIG requirement for FIPS, it can't comply with its actual *intent*. The profile author has modified the FIPS-mode kernel check (SV-238363) in this overlay profile to *fail by default* accordingly.
